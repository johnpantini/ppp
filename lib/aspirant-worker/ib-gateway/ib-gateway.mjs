...
const $$ibGateway = globalThis.ppp.$debug('ib-gateway');
...
class TwsConnection {
  ...
  // Observers are arrow functions so that `this` is the TwsConnection
  // instance and private fields resolve without binding.
  ...
  #errorObservable = {
    next: (error = {}) => {
      $$ibGateway(
        '[%s] TWS error, code: %s, reqId: %s, message: %s',
        this.#key,
        error.code,
        error.reqId,
        error.error?.message
  ...
  reqMktDepth(body = {}) {
    ...
    this.#api.api.reqMktDepth(tickerId, contract, numRows, isSmartDepth, []);
  ...
  reqTickByTick(body = {}) {
    ...
    this.#api.api.reqTickByTickData(reqId, contract, 'AllLast', 0, false);
  ...
  async connect(body = {}) {
    if (!this.#api) {
      $$ibGateway(
        '[%s] creating IBApiNext, host: %s, port: %s',
        this.#key,
        body.host ?? 'localhost',
        body.port ?? 7496
      );
    ...
    if (this.#connectionState === ConnectionState.Disconnected) {
      $$ibGateway('[%s] connecting to TWS...', this.#key);
  ...
  disconnect() {
    $$ibGateway('[%s] disconnecting from TWS', this.#key);
    ...
    this.#app
      .ws('/*', {
        ...
        close: (ws) => {
          ...
          $$ibGateway('[%s] client disconnected', ws.key);
        ...
        message(ws, message) {
          ...
          if (!ws.key) {
            ...
            $$ibGateway('[%s] client subscribed', ws.key);
            ws.subscribe(text);
            ws.send(
              JSON.stringify({
          ...
          try {
            ...
            $$ibGateway('[%s] WS command failed: %o', ws.key, e);
          }
        }
      })
      .post('/call', async (res) => {
        try {
          ...
          // errorResponse() logs the error itself.
      ...
      .listen(
        ...
        (listenSocket) => {
          if (listenSocket) {
            ...
            $$ibGateway('failed to get a port, terminating...');
            process.exit(1);
          }
        }
      );
