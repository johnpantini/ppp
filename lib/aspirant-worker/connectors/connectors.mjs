...
const $$connectors = globalThis.ppp.$debug('connectors');
    ...
    client
      .on('ready', () => {
        client.exec(body.cmd, { pty: true }, (err, stream) => {
          if (err) {
            $$connectors('ssh exec failed: %o', err);
      ...
      .on('error', (e) => {
        $$connectors('ssh connection error: %o', e);
...
async function redis(request, response) {
  ...
  try {
    ...
    client.on('error', (e) => {
      $$connectors('redis client error: %o', e);
    ...
    try {
      ...
      // quit() rejects when the connection is already closed (e.g. after
      // a connection error), which would crash the process.
      client.quit().catch(() => client.disconnect());
    }
  } catch (e) {
    if (errorOccurred) return;

    $$connectors('redis failed: %o', e);
...
}).listen(process.env.NOMAD_PORT_HTTP ?? process.env.PORT ?? 14535, () => {
  $$connectors('listening on port %d', server.address().port);
});
