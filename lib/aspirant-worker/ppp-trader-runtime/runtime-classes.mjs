  ...
  async #connectToURLTraderFromAspirantWorker(reconnect) {
    ...
    if (this.#awConnection?.readyState === WebSocket.OPEN) {
      ...
      this.#pendingAWConnectionPromise.then(() => {
        this.$$rawtr(
          '[%s] connection was established and this.#pendingAWConnectionPromise is now void 0',
          this.document.name
        );
...
async function trinityResponseToObject(response) {
  ...
  try {
    ...
    if (payload.T === 'connack') {
        ...
        if (trinityUrl) {
          ...
          if (trinityResponse.ok) {
            ...
            if (document && code && instruments) {
              ...
              trader.$$debug = globalThis.ppp.$debug(document._id);
              trader.$$rpc = trader.$$debug.extend('rpc');
              trader.$$rpc.log = console.log.bind(console);

              if (typeof trader.oneTimeInitializationCallback === 'function') {
                trader.$$rpc(
      ...
      try {
        ...
        const response = await ws.trader[payload.method](...payload.args);
