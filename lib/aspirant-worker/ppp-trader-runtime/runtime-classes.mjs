...
export async function onRuntimeWebSocketClose(ws) {
  ...
  if (ws.trader && this.wsSources.has(ws)) {
    ...
    this.$$ws('connection closed: %o', stripDocument(ws.trader.document));

    ws.trader = null;
  } else {
    this.$$ws('connection closed');
  }
}
...
export async function onRuntimeWebSocketMessage(ws, message) {
  try {
    ...
    if (payload.T === 'connack') {
      ...
      if (payload.trinityUrl && canUseRedis()) {
        this.$$server(
      ...
      if (typeof existingTrader !== 'undefined' && existingTrader !== true) {
        ...
        if (!ws.closed) {
          ...
          ws.trader.$$debug(
            'connack, trader is ready: %o',
            stripDocument(ws.trader.document)
          );
        }
      } else if (existingTrader === true) {
        // Another connection is creating this trader, wait for it
        // instead of creating a second instance.
        this.$$server('trader %s is being created, waiting...', payload._id);

        let trader = existingTrader;

        // Up to 60 seconds.
        for (let i = 0; i < 600 && trader === true && !ws.closed; i++) {
          await later(100);

          trader = this.traders.get(payload._id);
        }
        ...
        if (typeof trader === 'object' && trader !== null) {
          ws.trader = trader;

          ws.send(JSON.stringify([{ T: 'ready' }]));
          ws.trader.$$debug(
            'trader is ready after waiting: %o',
            stripDocument(ws.trader.document)
          );
        } else {
          ws.send(
            JSON.stringify([
              { T: 'error', code: 503, msg: 'trader creation failed.' }
            ])
          );
        }
      } else {
        ...
        try {
          // No trader here, trinity document is needed. Use the URL from
          // the payload, fall back to the cached one if Redis is available.
          const trinityUrl =
            payload.trinityUrl ??
            (canUseRedis()
              ? await this.redis.hget(this.getFullKey('trinity'), payload._id)
              : null);

          if (trinityUrl) {
            this.$$server('using trinityUrl for %s', payload._id);
            ...
            if (trinityResponse.ok) {
              ...
              if (document && code && instruments) {
                ...
                if (
                  ...
                  try {
                    globalThis.vm.runInThisContext(code, {
                      ...
                      importModuleDynamically:
                        globalThis.vm.constants.USE_MAIN_CONTEXT_DEFAULT_LOADER
                    });
                  } finally {
                  ...
                  if (
                    typeof trader.oneTimeInitializationCallback ===
                      'function' &&
          ...
          // Drop the stub if no trader has been created, so that the next
          // connack (and connections waiting above) can proceed.
          if (this.traders.get(payload._id) === true) {
            this.traders.delete(payload._id);
          }
        }
      }
      ...
      if (source && ws.trader) {
        ...
        try {
          ...
          // Reduce spam.
          if (exception?.message !== 'E_TRADER_IS_CLOSED') {
            ws.trader.$$debug(
