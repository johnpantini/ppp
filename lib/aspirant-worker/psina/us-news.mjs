...
export class PsinaUSNews {
  ...
  async fetchAstraRESTEndpoint(path, options = {}) {
    return (
      await fetch(
        `https://${
          new URL(process.env.GLOBAL_PROXY_URL).hostname
        }/api/rest/v2${path}`,
        {
          ...
          headers: Object.assign(
            {
              ...
              'X-Host': `${process.env.ASTRA_DB_ID}-${process.env.ASTRA_DB_REGION}.apps.astra.datastax.com`,
              'X-Allowed-Headers': 'X-Cassandra-Token,Content-Type'
            },
            options?.headers ?? {}
          ),
          body: options.body
