...
class FunctionsFactory {
  ...
  async callFunction(name, ...args) {
    ...
    body.mongoDbUri = localStorage.getItem('ppp-mongo-connection-uri');
...
class Credentials {
   ...
   * @param {string} providerType Type of the authentication provider.
   * @param {Record<string, unknown>} payload Provider-specific credential data.
   */
   ...
   *
   * @param {string} key Secret API key value.
   * @returns {Credentials} Credentials accepted by App.logIn.
   */
  static apiKey(key) {
...
class Authenticator {
  ...
  async authenticate(credentials, linkingUser) {
    const locationUrl = this.fetcher.locationUrlContext._locationUrl;
    const url = locationUrl.endsWith('/')
      ? locationUrl + 'mongodb'
      : locationUrl + '/mongodb';
    ...
    await fetch(url, {
      ...
      cache: 'reload',
      body: JSON.stringify({
        mongoDbUri: localStorage.getItem('ppp-mongo-connection-uri')
      })
    });
...
class MongoDBRealmError extends Error {
   ...
   * @param {string} statusText HTTP status description.
   * @param {string} [error] Provider error message.
   * @param {string} [errorCode] Provider error identifier.
   * @param {string} [link] Provider diagnostic URL.
   */
   ...
   *
   * @param {{method?: string, url: string}} request Request sent to the server.
   * @param {Response} response Raw response whose body will be consumed.
   * @returns {Promise<MongoDBRealmError>} Error with parsed provider and HTTP details.
   */
...
class Fetcher {
  ...
  async fetch(request) {
    ...
    if (typeof path === 'string' && typeof url === 'string') {
      ...
      const locationUrl = await this.locationUrlContext.locationUrl;
      const url = locationUrl.endsWith('/')
        ? locationUrl.slice(0, -1) + path
        : locationUrl + path;
...
class App {
   ...
   *
   * The local proxy URL must already be configured in localStorage.
   * @param {string | {id: string, baseUrl?: string, skipLocationRequest?: boolean, app?: object, storage?: object}} idOrConfiguration App ID or connection/storage configuration.
   */
  constructor(idOrConfiguration) {
     ...
     */
    this._locationUrl = localStorage
      .getItem('ppp-mongo-proxy-url')
   ...
   * Switch user.
   *
   * @param {User} nextUser Existing user instance belonging to this app.
   * @returns {void}
   * @throws {Error} When the instance does not belong to this app.
   */
  switchUser(nextUser) {
   ...
   * Log in a user.
   *
   * @param {Credentials} credentials Credentials passed to the configured authenticator.
   * @param {boolean} [fetchProfile=true] Fetch the user's profile after authentication.
   * @returns {Promise<User>} Authenticated user selected as current.
   */
   ...
   *
   * @returns {User | null} Most recently selected active user, or null.
   */
  get currentUser() {
   ...
   *
   * @returns {Record<string, User>} New ID-to-user mapping, retaining user instances.
   */
  get allUsers() {
    // The mapping is a fresh object; user instances are intentionally shared.
