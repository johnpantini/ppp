...
export const shouldUseAlternativeMongo =
  localStorage.getItem('ppp-use-alternative-mongo') === '1' &&
  localStorage.getItem('ppp-mongo-proxy-url')?.startsWith?.('http') &&
  localStorage.getItem('ppp-mongo-connection-uri')?.startsWith?.('mongodb');
...
class Fetcher {
  ...
  async fetch(request) {
    ...
    if (typeof path === 'string' && typeof url === 'string') {
      ...
      const globalProxyUrl = localStorage.getItem('ppp-global-proxy-url');

      let response;

      if (globalProxyUrl && !shouldUseAlternativeMongo) {
        response = await ppp.fetch(url, {
          ...restOfRequest,
          headers: {
 ...
 * @throws {FetchError}
 */
async function getMongoDBRealmAccessToken({
  username = ppp.keyVault.getKey('mongo-public-key'),
  apiKey = ppp.keyVault.getKey('mongo-private-key')
} = {}) {
  const rMongoDBRealmAccessToken = await ppp.fetch(
...
async function upsertMongoDBRealmScheduledTrigger({
  functionName,
  triggerName,
  functionSource,
  schedule = '*/5 * * * *'
  ...
  await maybeFetchError(
    rFunctionList,
    'Не удалось получить список облачных функций во время создания триггера.'
  );
  ...
  if (func) {
    ...
    funcId = jCreateFunc._id;
  }

  const rTriggerList = await ppp.fetch(
  ...
  await maybeFetchError(rTriggerList, 'Не удалось получить список триггеров.');

  const triggerList = await rTriggerList.json();
  const trigger = triggerList?.find((t) => t.name === triggerName);

  if (trigger) {
    const rUpdateTrigger = await ppp.fetch(
      `https://services.cloud.mongodb.com/api/admin/v3.0/groups/${groupId}/apps/${appId}/triggers/${trigger._id}`,
      {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${mongoDBRealmAccessToken}`,
    ...
    await maybeFetchError(rUpdateTrigger, 'Не удалось обновить триггер.');
  } else {
...
async function removeMongoDBRealmTrigger({ triggerName }) {
  const groupId = ppp.keyVault.getKey('mongo-group-id');
  const appId = ppp.keyVault.getKey('mongo-app-id');
  const mongoDBRealmAccessToken = await getMongoDBRealmAccessToken();
