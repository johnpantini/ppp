...
import { invalidate, maybeFetchError } from './ppp-errors.js';
...
class Fetcher {
  ...
  async fetch(request) {
    ...
    if (typeof path === 'string' && typeof url === 'string') {
      ...
      if (serviceMachineUrl && !shouldUseAlternativeMongo) {
...
async function enableMongoDBRealmHosting({
  groupId,
  appId,
  serviceMachineUrl,
...
async function upsertMongoDBRealmScheduledTrigger({
  ...
  if (trigger) {
    const rUpdateTrigger = await fetch(
      new URL('fetch', serviceMachineUrl).toString(),
      {
        cache: 'no-cache',
        method: 'POST',
        body: JSON.stringify({
          method: 'PUT',
          url: `https://realm.mongodb.com/api/admin/v3.0/groups/${groupId}/apps/${appId}/triggers/${trigger._id}`,
          body: {
            name: triggerName,
            type: 'SCHEDULED',
            disabled: false,
