let urlPath = location.pathname + location.search;
...
let locationHandlerAssigned = false;
...
window.addEventListener('message', (event) => {
  if (typeof event.data === 'string' && /paletteWhite/.test(event.data)) {
    ...
    if (saveLocation && !locationHandlerAssigned) {
      locationHandlerAssigned = true;
    ...
    const styles = css`
      * {
      ...
      .tv-screener-sticky-header-wrapper__fields-button-wrap {
        border-left-color: ${darkMode
          ? paletteGrayDark1
          : paletteGrayLight2} !important;
        border-top-color: ${darkMode
          ? paletteGrayDark1
          : paletteGrayLight2} !important;
      }
      ...
      ${options.compactMode
        ? `
        tr.tv-screener-table__result-row {
          height: 25px !important;
        }

         .tv-screener-table__symbol-container-description .tv-screener__description {
           display: none !important;
         }
         ...
         .tv-screener-table__head-left.tv-screener-table__head-left--name-col .tv-screener-table__field-value--total,
         .tv-screener-table__head-left.tv-screener-table__head-left--name-col .apply-common-tooltip {
           display: none !important;
         }

         .tv-screener-sticky-header-wrapper__fields-button,
         .tv-data-table__thead-row.tv-data-table__stroke.tv-screener-table__head {
           height: 38px !important;
         }
