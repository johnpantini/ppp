...
async function fetchViaProxy(url, options = {}, allowedHeaders = []) {
  process.env.DEBUG === 'true' && console.log('Fetching via proxy:', url);
          ...
          if (!reconnect && this.#socket) {
            ...
            if (this.#accessToken) {
              ...
              this.#socket.on('connect', () => {
                this.debug(
                  `${this.#host}:${this.#port} connection established.`
                );
              ...
              this.debug(
                `Reconnecting on failed authorization in ${
                  Math.max(this.#reconnectionTime, 3600000) / 1000
                }s...`
              );
