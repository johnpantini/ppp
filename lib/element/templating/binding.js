...
import { Observable, setCurrentEvent } from '../observation/observable.js';
...
function contentUnbind() {
  ...
  const view = this.target.$pppView;
...
function updateContentTarget(value) {
  ...
  if (value.create) {
    ...
    if (view === void 0) {
      ...
      if (this.target.$pppTemplate !== value) {
        if (view.isComposed) {
          view.remove();
          view.unbind();
        }
    ...
    if (!view.isComposed) {
      ...
      this.target.$pppView = view;
      this.target.$pppTemplate = value;
