...
export class MatchMediaStyleSheetBehavior extends MatchMediaBehavior {
   ...
   *
   * ```ts
   * import { css } from "element";
   * import { MatchMediaStyleSheetBehavior } from "wc";
   *
  ...
  constructListener(source) {
    ...
    return function listener() {
      ...
      if (matches && !attached) {
        source.$pppController.addStyles(styles);
        attached = matches;
      } else if (!matches && attached) {
        source.$pppController.removeStyles(styles);
        attached = matches;
      }
    };
  }
  ...
  unbind(source) {
    super.unbind(source);
    source.$pppController.removeStyles(this.styles);
  }
}
