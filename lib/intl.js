export function getInstrumentPrecision(instrument, price) {
  ...
  const [dec, frac] = (instrument.minPriceIncrement || derivedMinPriceIncrement)
    .toString()
    .split('.');
...
export function formatAmount(amount, currency, instrument) {
  ...
  const precision = getInstrumentPrecision(instrument, amount);
  ...
  if (
    ...
    return (
      new Intl.NumberFormat('ru-RU', {
        ...
        maximumFractionDigits: precision
      }).format(amount) + ` ${instrument.currency}`
    );
  }
...
export function formatAbsoluteChange(price, instrument) {
  ...
  return new Intl.NumberFormat('ru-RU', {
    style: 'decimal',
    minimumFractionDigits: getInstrumentPrecision(instrument, price),
    signDisplay: 'always'
  }).format(price);
}
