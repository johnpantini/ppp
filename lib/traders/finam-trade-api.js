...
class FinamTradeApiTrader extends Trader {
  ...
  async historicalCandles({ instrument, tf, from, to }) {
    ...
    const params = [
      ...
      'TimeFrame=M5',
      'Interval.Count=500',
      `Interval.To=${new Date().toISOString()}`
    ].join('&');
    const candlesResponse = await fetch(`${this.document.connectorUrl}fetch`, {
      method: 'POST',
      body: JSON.stringify({
        method: 'GET',
        url: `https://trade-api.finam.ru/public/api/v1/intraday-candles?${params}`,
        headers: {
          'X-Api-Key': this.document.broker.token
        }
      })
  ...
  async formatError({ error }) {
    ...
    if (details.error) {
      if (/Money shortage/i.test(details.error?.message)) {
        ...
        return 'Ошибка выбора маршрута исполнения.';
      } else if (
      ...
      } else if (/No enough coverage/i.test(details.error?.message)) {
