...
class RealOrderDatum extends AlorTraderGlobalDatum {
  firstReferenceAdded() {
    if (this.trader.connection.readyState === WebSocket.OPEN) {
      this.trader.connection.send(
        JSON.stringify({
...
class AlorOpenAPIV2Trader extends Trader {
      ...
      if (this.#pendingAccessTokenResponse) {
        ...
        this.#pendingAccessTokenResponse = fetch(
          ...
          .catch((e) => {
            ...
            // Propagate a failed retry, otherwise callers hang forever.
      ...
      this.#pendingConnection = new Promise((resolve) => {
        ...
        ws.onmessage = ({ data }) => {
          ...
          if (payload?.httpCode >= 400) {
            if (payload.httpCode === 401) {
              // The token was rejected by the server, get a new one.
          ...
          if (payload?.data && payload?.guid) {
            ...
            if (
              ...
              datumInstance instanceof RealOrderDatum
            ) {
        ...
        if (o.status === 'working' && (o.side === side || side === 'all')) {
          ...
          if (orderInstrument?.minPriceIncrement > 0) {
            ...
            fetch(
              ...
              .catch((e) => this.$$orders('error modifying order: %o', e));
          }
        }
      }
    } else {
    ...
    if (response.status === 200) {
      ...
      for (const o of orders) {
        if (o.status === 'working') {
          ...
          // Async way.
          this.cancelRealOrder(o).catch((e) =>
            this.$$orders('cancelRealOrder %s failed: %o', o.id, e)
          );
        }
      }
    } else {
  ...
  getErrorI18nKey({ instrument, error }) {
    ...
    if (
      /Нехватка средств по лимитам клиента/i.test(message) ||
      /Заявка приводит к возникновению непокрытого риска/i.test(message)
    ) {
