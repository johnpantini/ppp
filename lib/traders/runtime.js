...
export class TraderRuntime {
  ...
  async #onRemoteTraderMessage(data, done) {
        ...
        switch (payload.T) {
          ...
          case 'subscription-exception':
            if (payload.sourceKey) {
              ...
              if (typeof source !== 'undefined') {
                if (payload.exception.name === 'TypeError') {
                  source.catchException?.(
                    new TypeError(payload.exception.args.message)
                  );
                } else if (payload.exception.name === 'ReferenceError') {
                  source.catchException?.(
                    new ReferenceError(payload.exception.args.message)
                  );
                } else {
                  source.catchException?.(
                    new Exceptions[
                      payload.exception.name ?? 'RemoteTraderError'
  ...
  async #connectToRemoteTraderRuntime() {
    if (this.#awConnection?.readyState === WebSocket.OPEN) {
      ...
      this.#pendingAWConnectionPromise = new Promise((resolve) => {
        ...
        this.#awConnection.onclose = async () => {
          ...
          return this.#connectToRemoteTraderRuntime();
        };
        this.#awConnection.onerror = () => this.#awConnection.close();
        this.#awConnection.onmessage = ({ data }) =>
          this.#onRemoteTraderMessage(JSON.parse(data), () => {
            resolve(this.#awConnection);
          });
      });

      this.#pendingAWConnectionPromise.then(() => {
        this.#pendingAWConnectionPromise = void 0;
      });
