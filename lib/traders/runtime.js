...
export class TraderRuntime {
  ...
  // Generic error names (TypeError, Error...) are not exported from
  // ppp-exceptions.js, mirror the Aspirant Worker branch logic.
  #remoteExceptionToError(exception = {}) {
    const { name, args = {} } = exception;

    if (name === 'TypeError') {
      return new TypeError(args.message);
    } else if (name === 'ReferenceError') {
      return new ReferenceError(args.message);
    } else if (name in Exceptions) {
      return new Exceptions[name]({
        message: args.message,
        details: args.details
      });
    }

    return new Exceptions.RemoteTraderError({
      message: `[${name}] ${args.message}`
    });
  }

  // A rejection inside port.onmessage would be unhandled.
  async #getTraderSafely(document) {
    try {
      return await ppp.getOrCreateTrader(document);
    } catch (e) {
      this.$$runtime('getOrCreateTrader failed: %o', e);
  ...
  async load(options = {}) {
    ...
    if (this.#pendingDictionaryPromise) {
      ...
      this.#pendingDictionaryPromise = new Promise(async (resolve, reject) => {
        // An async executor cannot reject by throwing, do it explicitly.
        try {
          ...
          case 'shared-worker':
            ...
            this.#worker.port.onmessage = async ({ data }) => {
              if (data?.type === 'conn') {
                ...
                this.buildDictionary(resolve, reject).catch(reject);
                ...
                if (data.sourceKey) {
                  ...
                  if (typeof source !== 'undefined') {
                    source.catchException?.(
                      this.#remoteExceptionToError(data.exception)
                    );
                  }
                }
              } else if (data?.type === 'shared-worker-needed') {
                if (await this.#getTraderSafely(data.document)) {
                  data.port2.postMessage({ type: 'shared-worker-created' });
                }
              } else if (data?.type === 'shared-worker-subscribe-field') {
                const requestedSharedWorker = await this.#getTraderSafely(
                  data.document
                );
            ...
            for (const message of messages) {
              switch (message.T) {
                ...
                case 'subscription-exception':
                  ...
                  break;
                case 'a': {
                  if (typeof message.SI === 'string') {
                    // Source and field.
                    const source = this.#sources.get(
                      `${message.SI}:${message.f}`
                    );

                    if (typeof source !== 'undefined') {
                      source[message.f] = message.v;
                    }
                  } else if (message.M && typeof message.M === 'object') {
                    // Source to field map.
                    for (const sourceID in message.M) {
                      const field = message.M[sourceID];
                      const source = this.#sources.get(`${sourceID}:${field}`);

                      if (typeof source !== 'undefined') {
                        source[field] = message.v;
                      }
                    }
                  }
        ...
        return new Promise(async (resolve, reject) => {
          this.#bus.once(`rpc-${rid}`, ({ detail }) => {
            if (!('exception' in detail)) {
              ...
              } else if (detail.exception.name === 'ReferenceError') {
