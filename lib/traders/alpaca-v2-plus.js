...
class CandleDatum extends AlpacaV2PlusTraderDatum {
  filter(data, instrument, source, datum) {
    ...
    if (sup) {
      if (
        ...
        (typeof data.tf === 'string' || typeof data.tf === 'number')
      ) {
        let tf = parseInt(data.tf);

        if (typeof data.tf === 'string') {
          if (data.tf.endsWith('m')) {
            tf *= 60;
          } else if (data.tf.endsWith('h')) {
            tf *= 3600;
          }
        }

        return tf === parseInt(source.getAttribute('tf'));
      }
    }
  ...
  // The server stream is shared per symbol|tf: several charts with the same
  // symbol and timeframe use one subscription, so it is refcounted here.
  #streamRefs = new Map();

  // The tf a source subscribed with, the attribute may change later.
  #sourceStreams = new Map();

  async firstReferenceAdded(source, symbol) {
    const tf = source.getAttribute('tf');

    if (typeof tf !== 'string' && typeof tf !== 'number') {
      return;
    }

    const key = `${symbol}|${tf}`;
    const refs = this.#streamRefs.get(key) ?? 0;

    this.#streamRefs.set(key, refs + 1);
    this.#sourceStreams.set(source, key);

    if (refs === 0 && this.trader.connection.readyState === WebSocket.OPEN) {
      this.trader.connection.send(
        JSON.stringify({
          action: 'candles-subscribe',
          tf,
          symbol
        })
      );
  ...
  async anotherReferenceAdded(source, symbol) {
    // Multiple charts, same or different tf scenario.
    return this.firstReferenceAdded(source, symbol);
  }

  async lastReferenceRemoved(source, symbol) {
    const key = this.#sourceStreams.get(source);

    if (typeof key === 'undefined') {
      return;
    }

    this.#sourceStreams.delete(source);

    const refs = this.#streamRefs.get(key) ?? 0;

    if (refs > 1) {
      // Another chart still uses this stream.
      this.#streamRefs.set(key, refs - 1);

      return;
    }

    this.#streamRefs.delete(key);
  ...
  async anotherReferenceRemoved(source, symbol) {
...
class ComboDatum extends AlpacaV2PlusTraderDatum {
  ...
  [TRADER_DATUM.MARKET_PRINT](data) {
    ...
    const tradeId = `${data.S}|${side}|${data.p}|${data.s}|${pool}|${timestamp}`;

    if (this.trader.document.transferMode === 'compact') {
      return [tradeId, data.S, side, data.c, timestamp, data.p, data.s, pool];
    } else {
      return {
        tradeId,
        symbol: data.S,
...
class OrderbookDatum extends AlpacaV2PlusTraderDatum {
    ...
    if (this.trader.connection.readyState === WebSocket.OPEN) {
      ...
      if (this.trader.document.broker.type === BROKERS.ALPACA) {
        try {
          ...
          // The live subscription below must still be sent.
          this.trader.$$debug('latest quote failed: %o', e);
        }

        // Unsubscribed while the request was pending.
        if (!this.refs.get(symbol)) {
          return;
        }
      }
  ...
  [TRADER_DATUM.ORDERBOOK](orderbook, instrument) {
    ...
    const orderbookSymbol = this.trader.getSymbol(instrument);
    let orderbookMap = this.orderbooks.get(orderbookSymbol);

    // A quote may arrive before the map exists (e.g. during a reconnect).
    if (typeof orderbookMap === 'undefined') {
      ...
      this.orderbooks.set(orderbookSymbol, orderbookMap);
    }
  ...
  async establishWebSocketConnection(reconnect) {
    if (this.connection?.readyState === WebSocket.OPEN && this.authenticated) {
      ...
      this.#pendingConnection = new Promise((resolve, reject) => {
        ...
        const ws = new WebSocket(this.document.wsUrl);
        ...
        ws.onmessage = async ({ data }) => {
          ...
          // A single object frame must not break the loop below.
          const parsed = [].concat(JSON.parse(data) ?? []);
          ...
          for (const payload of parsed) {
            if (payload.msg === 'connected') {
              ...
              if (payload.code === 407) {
                // Slow client, skip for now.
              } else if (payload.code === 406) {
                // Connection limit exceeded: reconnecting would not help.
                this.authenticated = false;
                ws.onclose = null;
                ...
                // Auth failed, auth timeout or any error during the
                // handshake: fatal, no reconnection.
                this.authenticated = false;
                ws.onclose = null;
                ...
                // Subscription-level errors (405, 409, 410, 500...) after
                // auth must neither drop the session nor disable reconnection.
                this.$$connection?.('error frame: %o', payload);
              }
            }
          }
        };
  ...
  async historicalCandles({ instrument, unit, value, cursor }) {
    if (this.document.broker.type === BROKERS.PSINA) {
      try {
        ...
        this.$$debug('historicalCandles failed: %o', e);
