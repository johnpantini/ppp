...
class OrderbookDatum extends TinkoffTraderDatum {
  async firstReferenceAdded(source) {
    ...
    if (instrument && !instrument.notSupported && instrument.tinkoffFigi) {
      try {
        ...
        // The stream below still has to be (re)started.
        this.trader.$$debug('getOrderBook failed: %o', e);
      }
    }
  ...
  [TRADER_DATUM.ORDERBOOK](data, instrument) {
    let limitDownPrice = +(toNumber(data.limitDown) ?? 0).toFixed(
      getInstrumentPrecision(instrument)
    );
    let limitUpPrice = +(toNumber(data.limitUp) ?? 0).toFixed(
      getInstrumentPrecision(instrument)
    );
...
class PositionsDatum extends GlobalTraderDatum {
  ...
  async firstReferenceAdded() {
    try {
      ...
      // The streams below still have to be (re)started.
      this.trader.$$debug('portfolio snapshot failed: %o', e);
    }
...
class TinkoffGrpcWebTrader extends Trader {
  ...
  async oneTimeInitializationCallback() {
    try {
      ...
      // Only futures price display depends on this list.
      this.$$debug('futures list failed: %o', e);
    }
  }
  ...
  async #resubscribeToMarketDataStream() {
    if (
      ...
      // Nothing to stream, stop the previous one.
  ...
  async getOrders(client) {
    ...
    for (const o of (
      ...
      if (
        ...
        this.#futureMinPriceIncrementAmount.get(instrument.symbol) > 0
      ) {
  ...
  async estimate(instrument, price) {
    ...
    return {
      marginSellingPowerQuantity: maxLots.sellMarginLimits?.sellMaxLots ?? 0,
      marginBuyingPowerQuantity: maxLots.buyMarginLimits?.buyMaxLots ?? 0,
      sellingPowerQuantity: maxLots.sellLimits?.sellMaxLots ?? 0,
      buyingPowerQuantity: maxLots.buyLimits?.buyMaxLots ?? 0
    };
  }
