import { TRADER_DATUM, EXCHANGE, ORDERS } from '../const.js';
import {
...
class ConditionalOrderDatum extends TraderDatum {}
...
class Trader {
  ...
  conditionalOrderChanged(order) {
    for (const [source, field] of this.datums[TRADER_DATUM.CONDITIONAL_ORDER]
      .sources[TRADER_DATUM.CONDITIONAL_ORDER]) {
      this.assignSourceField(source, field, order);
    }
  }
...
class RemoteTraderRuntime {
  ...
  async subscribeField({ source, field, datum }) {
    ...
    } else if (this.document.runtime === 'aspirant-worker') {
    }
  }
