...
class Trader {
  ...
  async placeConditionalOrder({
    ...
    if (typeof implUrl === 'string') {
      if (singleton === 'global') {
        orderInstance = this.#conditionalOrders.find(
          (o) => o.orderId === payload.orderId
        );
      } else if (singleton === 'instrument') {
        orderInstance = this.#conditionalOrders.find(
          (o) =>
            o.payload.orderId === payload.orderId &&
      ...
      if (!orderInstance) {
        ...
        orderInstance = new instanceType();
