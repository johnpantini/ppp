...
class UtexMarginStocksTrader extends USTrader {
  ...
  async historicalCandles({ instrument, tf, from, to }) {
    if (typeof to === 'undefined') to = Date.now() / 1000;
    ...
    return {
      candles:
        data.candles?.reverse().map((c) => {
          return {
            open: +(+c.open / 1e8).toFixed(getInstrumentPrecision(instrument)),
            high: +(+c.high / 1e8).toFixed(getInstrumentPrecision(instrument)),
            low: +(+c.low / 1e8).toFixed(getInstrumentPrecision(instrument)),
            close: +(+c.close / 1e8).toFixed(
