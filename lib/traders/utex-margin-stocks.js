...
class QuotesDatum extends UtexMarginStocksTraderDatum {
  ...
  async lastReferenceRemoved(source, symbol) {
    ...
    // Do not let the maps grow with every instrument change.
    this.instrumentToI.delete(symbol);

    if (typeof i !== 'undefined') {
      this.iToInstrument.delete(i);
    }
  }
...
class TimelineDatum extends UtexMarginStocksTraderGlobalDatum {
  ...
  [TRADER_DATUM.TIMELINE_ITEM](data) {
    ...
    const commission =
      (((data.tradePrice ?? data.price) / 1e8) *
        data.tradeQty *
        instrument.lot *
        commissionRate) /
      100;
...
class RealOrderDatum extends UtexMarginStocksTraderGlobalDatum {
  valueKeyForData(data) {
...
class UtexMarginStocksTrader extends USTrader {
  ...
  get orders() {
    return Array.from(this.datums[TRADER_DATUM.REAL_ORDER].value);
  }
  ...
  async serialize() {
    ...
    return {
      ...
      leftMarginBP: this.leftMarginBP,
      usedMarginBP: this.usedMarginBP
    };
  }
  ...
  async ensureAccessTokenIsOk() {
    try {
      ...
      if (this.#pendingAccessTokenRequest) {
        ...
        this.#pendingAccessTokenRequest = new Promise((resolve, reject) =>
          (async () => {
            ...
            if (isJWTTokenExpired(savedAccessToken)) {
              if (isJWTTokenExpired(savedRefreshToken)) {
                const tokensResponse = await globalThis.ppp.fetch(
                ...
                const refreshAuthRequest = await globalThis.ppp.fetch(
      ...
      this.$$debug('[UTEX] ensureAccessTokenIsOk failed: %o', e);

      this.#pendingAccessTokenRequest = void 0;

      // A blocked account must not be retried in a loop either.
      if (e instanceof AuthorizationError || e instanceof UTEXBlockError) {
        throw e;
      }
      ...
      return new Promise((resolve, reject) => {
        setTimeout(
          () => {
            this.ensureAccessTokenIsOk().then(resolve, reject);
          },
  ...
  async establishWebSocketConnection(reconnect) {
    ...
    if (this.connection?.readyState === WebSocket.OPEN) {
      ...
      this.#pendingConnection = new Promise((resolve) => {
        const ws = new WebSocket('wss://ususdt-api-margin.utex.io/ws');
        ...
        ws.onclose = () => {
          ...
          // a second socket. The token is refreshed before reconnecting,
          // as before.
          const next = later(
            Math.max(this.document.reconnectTimeout ?? 1000, 1000)
          ).then(async () => {
        ...
        ws.onmessage = async ({ data }) => {
          ...
          if (payload.t === 12) {
            ...
            // The socket may have been closed or replaced during the await.
  ...
  async cancelRealOrder(order) {
    if (order.orderType === 'limit') {
      ...
      const request = await globalThis.ppp.fetch(
  ...
  async placeLimitOrder({ instrument, price, quantity, direction }) {
    ...
    const orderRequest = await globalThis.ppp.fetch(
  ...
  async estimate(instrument, price, quantity) {
    ...
    const marginBPQuantity =
      price > 0 ? Math.trunc(this.leftMarginBP / price / instrument.lot) : 0;
  ...
  async historicalCandles({ instrument, unit, value, cursor }) {
    ...
    const response = await globalThis.ppp.fetch(
      'https://ususdt-api-margin.utex.io/rest/grpc/com.unitedtraders.luna.utex.protocol.mobile.MobileDataService.getCandlesToDate',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json;charset=UTF-8',
    ...
    for (let i = data.candles.length - 1; i >= 0; i--) {
      const time = new Date(+data.candles[i].time * 1000);
      ...
      candles.push({
        open: +(+data.candles[i].open / 1e8).toFixed(
          getInstrumentPrecision(instrument)
        ),
        high: +(+data.candles[i].high / 1e8).toFixed(
          getInstrumentPrecision(instrument)
        ),
        low: +(+data.candles[i].low / 1e8).toFixed(
          getInstrumentPrecision(instrument)
        ),
        close: +(+data.candles[i].close / 1e8).toFixed(
          getInstrumentPrecision(instrument)
        ),
    ...
    return {
      cursor: data.candles?.[data.candles.length - 1]?.time,
      candles
    };
  }
  ...
  getErrorI18nKey({ error }) {
    if (error instanceof AuthorizationError) {
      return 'E_AUTHORIZATION_ERROR';
    }

    if (error instanceof UTEXBlockError) {
      return 'E_UTEX_BLOCK_ERROR';
    }
