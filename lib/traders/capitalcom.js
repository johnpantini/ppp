...
class CapitalcomTrader extends Trader {
  ...
  async ensureSessionIsOk() {
    try {
      ...
      if (this.#pendingSessionPromise) {
        ...
        this.#pendingSessionPromise = fetch(
          ...
          .then((json) => {
            ...
            // Connections interruptions should be skipped.
            if (json.headers && json.response && (!securityToken || !cst)) {
          ...
          .catch((e) => {
            this.trader.$$debug(
              '#pendingSessionPromise fetch failed: %s',
              e.message
            );
      ...
      this.trader.$$debug('ensureSessionIsOk failed: %s', e.message);
