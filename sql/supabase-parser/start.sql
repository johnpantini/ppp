...
begin
  while now() < end_time loop
    perform from dblink('dbname=[%#ctx.document.supabaseApi.db%] port=[%#ctx.document.supabaseApi.port%] host=[%#ctx.document.supabaseApi.hostname%] user=[%#ctx.document.supabaseApi.user%] password=[%#ctx.document.supabaseApi.password%]', 'select ppp_perform_job_[%#ctx.document._id%]();') as (res text);
    perform pg_sleep(1);
  end loop;
...
select dblink_connect('ppp-[%#ctx.document._id%]', 'dbname=[%#ctx.document.supabaseApi.db%] port=[%#ctx.document.supabaseApi.port%] host=[%#ctx.document.supabaseApi.hostname%] user=[%#ctx.document.supabaseApi.user%] password=[%#ctx.document.supabaseApi.password%]');
