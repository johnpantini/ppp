...
if [[ -z "${SYSTEMD_EXEC_PID}" ]]; then
  ...
  curl -s -XPOST http://127.0.0.1:8080/nginx/internal/resurrect -d "{\"ASPIRANT_ID\":\"$ASPIRANT_ID\",\"SERVICE_MACHINE_URL\":\"$SERVICE_MACHINE_URL\",\"REDIS_HOST\":\"$REDIS_HOST\",\"REDIS_PORT\":\"$REDIS_PORT\",\"REDIS_TLS\":\"$REDIS_TLS\",\"REDIS_REDIS_USERNAME\":\"$REDIS_REDIS_USERNAME\",\"REDIS_PASSWORD\":\"$REDIS_PASSWORD\",\"REDIS_DATABASE\":\"$REDIS_DATABASE\"}"
fi
